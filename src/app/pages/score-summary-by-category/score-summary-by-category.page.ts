...
@Component({
  selector: 'app-score-summary-by-category',
  templateUrl: './score-summary-by-category.page.html',
  styleUrls: ['./score-summary-by-category.page.scss'],
})
export class ScoreSummaryByCategoryPage implements OnInit {
  ...
  traitObj = {
    Honest: { Traitid: 301, name: 'Honesty', score: 0, img: 'assets/images/round-trait-circle/honest.png' },
    Honorable: { Traitid: 302, name: 'Honorable', score: 0, img: 'assets/images/round-trait-circle/confidence.png' },
    Respectful: { Traitid: 303, name: 'Respectful', score: 0, img: 'assets/images/round-trait-circle/respectful.png' },
    Fair: { Traitid: 304, name: 'Fair', score: 0, img: 'assets/images/round-trait-circle/fair.png' },
    Forgiving: { Traitid: 305, name: 'Forgiving', score: 0, img: 'assets/images/round-trait-circle/forgive.png' },
    Generous: { Traitid: 306, name: 'Generous', score: 0, img: 'assets/images/round-trait-circle/generous.png' },
    Courageous: { Traitid: 307, name: 'Courageous', score: 0, img: 'assets/images/round-trait-circle/courageous.png' },
    Polite: { Traitid: 308, name: 'Polite', score: 0, img: 'assets/images/round-trait-circle/adaptability.png' },
    Loving: { Traitid: 309, name: 'Loving', score: 0, img: 'assets/images/round-trait-circle/compassion.png' },
    Trustworthy: { Traitid: 310, name: 'Trustworthy', score: 0, img: 'assets/images/round-trait-circle/loyalty.png' }
  };
  ...
  ngOnInit() {
    ...
    this.getScoreSummarybyCategory();
  }
  ...
  tempTraitslst = [];
  getScoreSummarybyCategory() {
    this.helperService.showLoader();
    let obj = {
      UserId: this.paramData.userId ? this.paramData.userId : this.userData.UserID,
      CatId: this.paramData.id
    }
    this.service.callServiceFunction(ServicesConstant.USER_MGMT.SCORE_SUMMARY_BY_CATEGORY, obj).subscribe((res: any) => {
        ...
        if (!res.data) {
          ...
          for (var property in this.traitObj) {
            if (this.traitObj.hasOwnProperty(property)) {
              this.tempTraitslst.push(this.traitObj[property]);
            }
          }
        }
        if (res.data) {
  ...
  customizeResponseData(data) {
    ...
    for (var property in this.traitObj) {
      if (this.traitObj.hasOwnProperty(property)) {
        this.serverResponse.Traitslst.forEach(element => {
          if (this.traitObj[property].Traitid == element.TraitsId) {
            ...
            element.name = this.traitObj[property].name;
          }
        });
      }
    }
    for (var prop in this.categoryObj) {
      this.serverResponse.ScorebyCategourylst.forEach(element => {
        if (this.categoryObj[prop].id == element.CatId) {
          this.categoryObj[prop].score = element.CatScore;
        }
      });
    }
    this.serverResponse.TopCharactifierslst.forEach((element, i) => {
    ...
    if (!this.serverResponse.Traitslst.length) {
      for (var property in this.traitObj) {
        if (this.traitObj.hasOwnProperty(property)) {
          let obj: any = {
            name: this.traitObj[property].name,
            TraitsScore: this.traitObj[property].score,
            img: this.traitObj[property].img
          }
          this.serverResponse.Traitslst.push(obj);
        }
      }
    }
  }
